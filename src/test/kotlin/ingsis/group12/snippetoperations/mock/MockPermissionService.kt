...
class MockPermissionService : PermissionService {
    ...
    override fun getUserPermissionByAssetId(
        ...
        return ResponseEntity.ok(SnippetPermission("owner", assetId, userId, "userName"))
    }

    override fun getUserPermissionsByUserId(userId: String): ResponseEntity<List<Permission>> {
        return ResponseEntity.ok(
            listOf(
                SnippetPermission("owner", UUID.randomUUID(), userId, "userName"),
                SnippetPermission("viewer", UUID.randomUUID(), userId, "userName"),
            ),
        )
    }
